...
@router.post("", response_model=ApiKeyRead, dependencies=[Depends(auth), Depends(permissions(P.ApiKey.CREATE))])
...
@router.get("", response_model=list[ApiKeyRead], dependencies=[Depends(auth), Depends(permissions(P.ApiKey.READ))])
...
@router.get(
    "/{api_key_id}", response_model=ApiKeyRead, dependencies=[Depends(auth), Depends(permissions(P.ApiKey.READ))]
)
async def get(api_key_id: UUID, service: ApiKeysService = Injected(ApiKeysService)):
    return await service.get(api_key_id)


@router.delete("/{api_key_id}", dependencies=[Depends(auth), Depends(permissions(P.ApiKey.DELETE))])
...
@router.patch(
    "/{api_key_id}", response_model=ApiKeyRead, dependencies=[Depends(auth), Depends(permissions(P.ApiKey.UPDATE))]
)
async def update(
    request: Request, api_key_id: UUID, api_key_data: ApiKeyUpdate, service: ApiKeysService = Injected(ApiKeysService)
):
