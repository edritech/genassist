...
@router.post("", response_model=CustomerRead, dependencies=[Depends(auth), Depends(permissions(P.Customer.CREATE))])
...
@router.get("", response_model=list[CustomerRead], dependencies=[Depends(auth), Depends(permissions(P.Customer.READ))])
async def get_all(skip: int = 0, limit: int = 20, service: CustomersService = Injected(CustomersService)):
    return await service.get_all(skip, limit)


@router.get(
    "/{customer_id}", response_model=CustomerRead, dependencies=[Depends(auth), Depends(permissions(P.Customer.READ))]
)
async def get(customer_id: UUID, service: CustomersService = Injected(CustomersService)):
    return await service.get(customer_id)


@router.delete("/{customer_id}", dependencies=[Depends(auth), Depends(permissions(P.Customer.DELETE))])
...
@router.patch(
    "/{customer_id}", response_model=CustomerRead, dependencies=[Depends(auth), Depends(permissions(P.Customer.UPDATE))]
)
async def update(
    customer_id: UUID, customer_data: CustomerUpdate, service: CustomersService = Injected(CustomersService)
):
