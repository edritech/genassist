...
@router.get(
    "", response_model=List[FeatureFlagRead], dependencies=[Depends(auth), Depends(permissions(P.FeatureFlag.READ))]
)
...
@router.get(
    "/{flag_id}", response_model=FeatureFlagRead, dependencies=[Depends(auth), Depends(permissions(P.FeatureFlag.READ))]
)
...
async def create_feature_flag(dto: FeatureFlagCreate, svc: FeatureFlagService = Injected(FeatureFlagService)):
