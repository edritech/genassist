...
async def store_oauth_code(
    ...
    try:
        ...
        if not token_data:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Failed to exchange authorization code for token"
            )
        ...
        return GmailAuthResponse(status="success", message="Gmail authentication successful. Tokens saved.")
...
async def save_gmail_token_to_data_source(
    ...
    try:
        ...
        if not ds:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Data source with ID {data_source_id} not found"
            )
