...
@router.get("", response_model=list[LlmAnalyst], dependencies=[Depends(auth), Depends(permissions(P.LlmAnalyst.READ))])
...
@router.get(
    "/{llm_analyst_id}",
    response_model=LlmAnalyst,
    dependencies=[Depends(auth), Depends(permissions(P.LlmAnalyst.READ))],
)
...
@router.post("", response_model=LlmAnalyst, dependencies=[Depends(auth), Depends(permissions(P.LlmAnalyst.CREATE))])
...
@router.patch(
    "/{llm_analyst_id}",
    response_model=LlmAnalyst,
    dependencies=[Depends(auth), Depends(permissions(P.LlmAnalyst.UPDATE))],
)
async def update(
    llm_analyst_id: UUID, data: LlmAnalystUpdate, service: LlmAnalystService = Injected(LlmAnalystService)
):
    return await service.update(llm_analyst_id, data)


@router.delete("/{llm_analyst_id}", dependencies=[Depends(auth), Depends(permissions(P.LlmAnalyst.DELETE))])
