...
async def discover_mcp_tools(request: DiscoverToolsRequest) -> DiscoverToolsResponse:
    ...
    try:
        ...
        if connection_type_raw not in ("stdio", "sse", "http"):
            raise ValueError(f"Invalid connection_type: {connection_type_raw}. Must be one of: 'stdio', 'sse', 'http'")
    ...
    try:
        ...
        if not api_key:
            return JSONRPCResponse(
                ...
                id=request.id,
                result=None,
                error={"code": -32001, "message": "Missing API key in Authorization header"},
            )
        ...
        if not mcp_server:
            return JSONRPCResponse(
                jsonrpc="2.0", id=request.id, result=None, error={"code": -32001, "message": "Invalid API key"}
            )

        # Check if server is active
        if mcp_server.is_active != 1:
            return JSONRPCResponse(
                jsonrpc="2.0", id=request.id, result=None, error={"code": -32002, "message": "MCP server is inactive"}
            )
        ...
        if request.method == "tools/list":
            ...
            return JSONRPCResponse(jsonrpc="2.0", id=request.id, result=tools_list, error=None)
            ...
            if not tool_name:
                return JSONRPCResponse(
                    ...
                    id=request.id,
                    result=None,
                    error={"code": -32602, "message": "Missing 'name' parameter in params"},
                )
            ...
            try:
                ...
                return JSONRPCResponse(jsonrpc="2.0", id=request.id, result=result_data, error=None)

            except ValueError as e:
                # Tool not found or validation error
                return JSONRPCResponse(
                    jsonrpc="2.0", id=request.id, result=None, error={"code": -32601, "message": str(e)}
                )
            except Exception as e:
                ...
                return JSONRPCResponse(
                    ...
                    id=request.id,
                    result=None,
                    error={"code": -32603, "message": f"Workflow execution failed: {str(e)}"},
                )
...
async def handle_sse(
    ...
    api_key = extract_bearer_token(authorization)
    if not api_key:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing API key in Authorization header")
    ...
    # Check if server is active
    if mcp_server.is_active != 1:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="MCP server is inactive")
...
async def handle_sse_messages(
    ...
    await transport.handle_post_message(
        scope,
        request.receive,
        asgi_send,  # type: ignore[arg-type]
    )
