...
async def get_pipeline_configs(
    model_id: UUID, service: MLModelPipelineConfigService = Injected(MLModelPipelineConfigService)
):
...
async def get_pipeline_config(
    model_id: UUID, config_id: UUID, service: MLModelPipelineConfigService = Injected(MLModelPipelineConfigService)
):
...
async def get_pipeline_run(
    model_id: UUID, run_id: UUID, service: MLModelPipelineRunService = Injected(MLModelPipelineRunService)
):
...
async def get_pipeline_artifacts(
    model_id: UUID, run_id: UUID, service: MLModelPipelineArtifactService = Injected(MLModelPipelineArtifactService)
):
...
async def download_artifact(
    ...
    try:
        ...
        response = FileResponse(path=safe_serving_path, filename=artifact.artifact_name, media_type=media_type)
        return response
