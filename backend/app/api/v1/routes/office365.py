...
async def office365_callback(
    ...
    try:
        ...
        if not app_settings_id:
            raise HTTPException(status_code=400, detail="App settings ID not found in data source")
        ...
        return Office365AuthResponse(status="success", message="Office365 authentication successful. Tokens saved.")
...
async def save_office365_token_to_data_source(
    ds_service: DataSourceService, data_source_id: str, token_data: Dict[str, Any], user_email: str, redirect_uri: str
):
    try:
        ...
        ds_update = DataSourceUpdate(name=ds.name, source_type=ds.source_type, is_active=1, connection_data=token_data)
...
async def get_office365_access_token(
    ...
    try:
        ...
        if not tenant_id or not client_id or not client_secret:
            raise HTTPException(status_code=500, detail="Microsoft credentials not configured in app settings")
...
async def get_office365_user_email(access_token: str) -> str:
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(f"Failed to retrieve Office365 user info: {response.status_code} - {response.text}")
            raise HTTPException(status_code=400, detail="Failed to retrieve user info from Microsoft Graph")

        user_info = response.json()
        user_email = user_info.get("mail") or user_info.get("userPrincipalName")

        if not user_email:
            raise HTTPException(status_code=400, detail="Email not found in Microsoft Graph user profile")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retrieve user email: {str(e)}")
