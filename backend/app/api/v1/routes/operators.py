...
@router.post(
    "",
    status_code=201,
    response_model=OperatorReadAfterCreate,
    dependencies=[Depends(auth), Depends(permissions(P.Operator.UPDATE))],
)
...
@router.get("", response_model=list[OperatorRead], dependencies=[Depends(auth), Depends(permissions(P.Operator.READ))])
...
@router.get(
    "/{operator_id}", response_model=OperatorRead, dependencies=[Depends(auth), Depends(permissions(P.Operator.READ))]
)
