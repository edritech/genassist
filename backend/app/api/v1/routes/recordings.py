...
@router.post("/analyze_recording", dependencies=[Depends(auth), Depends(permissions(P.Recording.CREATE_ANALYZE))])
...
@router.post(
    "/upload_transcript", dependencies=[Depends(auth), Depends(permissions(P.Recording.CREATE_UPLOAD_TRANSCRIPT))]
)
async def process_transcript(model: ConversationTranscriptCreate, service: AudioService = Injected(AudioService)):
...
@router.get("/recordings/{recording_id}", dependencies=[Depends(auth), Depends(permissions(P.Recording.READ))])
...
@router.post("/ask_question", dependencies=[Depends(auth), Depends(permissions(P.Recording.CREATE_ASK_QUESTION))])
...
@router.get("/files/{rec_id}", dependencies=[Depends(auth), Depends(permissions(P.Recording.READ_FILES))])
async def serve_file(rec_id: UUID, service: AudioService = Injected(AudioService)):
    ...
    safe_path = get_safe_file_path(recording_data.file_path, settings.RECORDINGS_DIR)
...
@router.get("/metrics", dependencies=[Depends(auth), Depends(permissions(P.Recording.READ_METRICS))])
