...
@router.get("", response_model=List[RoleRead], dependencies=[Depends(auth), Depends(permissions(P.Role.READ))])
...
@router.get("/{role_id}", response_model=RoleRead, dependencies=[Depends(auth), Depends(permissions(P.Role.READ))])
async def get(role_id: UUID, service: RolesService = Injected(RolesService)):
    return await service.get_by_id(role_id)


@router.post("", response_model=RoleRead, dependencies=[Depends(auth), Depends(permissions(P.Role.CREATE))])
async def create(role: RoleCreate, service: RolesService = Injected(RolesService)):
    return await service.create(role)


@router.patch("/{role_id}", response_model=RoleRead, dependencies=[Depends(auth), Depends(permissions(P.Role.UPDATE))])
...
@router.delete("/{role_id}", dependencies=[Depends(auth), Depends(permissions(P.Role.DELETE))])
