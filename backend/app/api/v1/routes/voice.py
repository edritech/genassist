...
@router.get(
    "/openai/session",
    summary="Get a temporary session key for OpenAI API",
    dependencies=[Depends(auth), Depends(permissions(P.Conversation.CREATE_IN_PROGRESS))],
)
async def get_openai_session_key(
    lang_code: str = Query(default=""), input_audio_format: str = Query(default="pcm16")
) -> str:
    """
    ...
    payload = {
        ...
        "input_audio_transcription": {"model": model, "prompt": "", "language": lang_code},
        "turn_detection": {
            "type": "server_vad",
            "threshold": 0.5,
...
@router.get(
    "/run-s3-audio-sync/{item_id}",
    status_code=200,
    summary="Trabscribe the content of S3 Bucket  defined Datasource",
