...
async def create_workflow_from_wizard(
    ...
    url = request.url._url.replace("/api/workflow-manager/config/from-wizard", f"/ai-agents/workflow/{agent_id}")
    ...
    workflow_record.executionState = workflow_engine.generate_execution_state_from_wizard(
        input_data=json_workflow
    )  # required not to crash Studio
...
@router.get("/config/available-node-types", dependencies=[Depends(auth)])
