...
MODELS_FOR_DOWNLOAD: List[str] = [f"sentence-transformers/{model['value']}" for model in EMBEDDING_MODELS]

# For form schemas - vector database options (short names)
FORM_OPTIONS_VECTOR: List[Dict[str, str]] = [
    {"value": model["value"], "label": model["label"]} for model in EMBEDDING_MODELS
]

# For form schemas - LEGRA options (full paths with sentence-transformers/ prefix)
FORM_OPTIONS_LEGRA: List[Dict[str, str]] = [
    {"value": f"sentence-transformers/{model['value']}", "label": model["label"]} for model in EMBEDDING_MODELS
]

# For form schemas - Bedrock options
FORM_OPTIONS_BEDROCK: List[Dict[str, str]] = [
    {"value": model["value"], "label": model["label"]} for model in BEDROCK_EMBEDDING_MODELS
]
