...
def calculate_word_count(transcript_segments: list[TranscriptSegmentInput], target: str) -> int:
    return sum(len(seg.text.split()) for seg in transcript_segments if seg.speaker.lower() == target)
...
def calculate_incremental_word_counts(
    ...
    existing_agent_words = round((existing_agent_ratio / 100) * existing_word_count) if existing_word_count > 0 else 0
    existing_customer_words = existing_word_count - existing_agent_words

    # Calculate cumulative totals
    existing_agent_words + incremental_agent_words
...
def get_messages_string_by_type(
    ...
    if exclude_fields:
        ...
        filtered_segments = [
            {k: v for k, v in segment.items() if k not in exclude_fields} for segment in filtered_segments
        ]
...
async def set_url_content_if_no_rag(item: KBBase):
    ...
    if not vector_db.get("enabled", False):
        ...
        headers = item.extra_metadata.get("http_headers") or item.extra_metadata.get("headers", {})
...
def extract_sub_messages(transcript: str, num_messages_to_extract: int = 2) -> str:
    ...
    last_items = data[-num_messages_to_extract:] if len(data) >= num_messages_to_extract else data

    # Step 3: Convert back to JSON string
    return json.dumps(last_items, indent=2)


async def make_async_web_call(method: str, url: str, headers: dict[str, str], payload: dict[str, Any]):
    """Make an asynchronous web call to a given URL with a given method, headers, and payload."""
    async with aiohttp.ClientSession(
        connector=aiohttp.TCPConnector(ssl=os.getenv("USE_SSL", "false").lower() == "true")
    ) as session:
...
def filter_conversation_messages_create_time(conversation_filter: ConversationFilter, query):
