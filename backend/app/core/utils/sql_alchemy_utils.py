...
def resolve_sort_column(model: Type[DeclarativeBase], field: SortField) -> InstrumentedAttribute:
    """
    ...
    try:
        ...
        raise ValueError(f"{model.__name__} has no column named {field.value!r}") from exc
    return column


def add_dynamic_ordering(model: Base, filter: BaseFilterModel, query):
    column = resolve_sort_column(model, filter.order_by)
    order_clause = desc(column) if filter.sort_direction == SortDirection.DESC else asc(column)
