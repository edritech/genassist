...
class LlmAnalystModel(Base):
    ...
    llm_provider = relationship(
        "LlmProvidersModel", back_populates="llm_analysts", foreign_keys=[llm_provider_id], uselist=False
    )


class LlmProvidersModel(Base):
    __tablename__ = "llm_providers"
    __table_args__ = (PrimaryKeyConstraint("id", name="llm_providers_pk"),)
    ...
    llm_analysts = relationship(
        "LlmAnalystModel", back_populates="llm_provider", foreign_keys=[LlmAnalystModel.llm_provider_id]
    )
