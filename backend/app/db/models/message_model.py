...
class TranscriptMessageModel(Base):
    ...
    conversation_id: Mapped[UUID] = mapped_column(
        SQLAlchemyUUID, ForeignKey("conversations.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    feedback: Mapped[list["MessageFeedbackModel"]] = relationship(
        "MessageFeedbackModel", back_populates="message", cascade="all, delete-orphan"
    )
...
class MessageFeedbackModel(Base):
    ...
    message_id: Mapped[UUID] = mapped_column(
        SQLAlchemyUUID, ForeignKey("transcript_messages.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    # Relationship
    message: Mapped["TranscriptMessageModel"] = relationship("TranscriptMessageModel", back_populates="feedback")
