...
class MLModelPipelineConfig(Base):
    ...
    pipeline_runs = relationship("MLModelPipelineRun", back_populates="pipeline_config", cascade="all, delete-orphan")
...
class MLModelPipelineRun(Base):
    ...
    pipeline_config_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("ml_model_pipeline_configs.id", ondelete="CASCADE"), nullable=False
    )
    workflow_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("workflows.id", ondelete="CASCADE"), nullable=False
    )
    ...
    workflow = relationship("WorkflowModel", lazy="selectin")
    artifacts = relationship("MLModelPipelineArtifact", back_populates="pipeline_run", cascade="all, delete-orphan")
...
class MLModelPipelineArtifact(Base):
    ...
    pipeline_run_id: Mapped[UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("ml_model_pipeline_runs.id", ondelete="CASCADE"), nullable=False
    )
    artifact_type: Mapped[ArtifactType] = mapped_column(
        SQLEnum(ArtifactType, name="artifact_type_enum", create_constraint=True), nullable=False
    )
