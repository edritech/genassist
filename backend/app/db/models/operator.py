...
class OperatorModel(Base):
    ...
    # relationships
    operator_statistics = relationship("OperatorStatisticsModel", back_populates="operator", uselist=False)
...
class OperatorStatisticsModel(Base):
    ...
    __table_args__ = (PrimaryKeyConstraint("id", name="operator_statistics_pkey"),)
    ...
    avg_customer_satisfaction: Mapped[Optional[decimal.Decimal]] = mapped_column(
        Numeric(5, 2), server_default=text("0.0")
    )
