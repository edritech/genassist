...
async def seed_data(session: AsyncSession, injector: Injector):
    ...
    operator = UserModel(
        ...
        is_active=1,
        hashed_password=pwd_context.hash(seed_operator_password),
        user_type_id=interactive_user_type.id,
        force_upd_pass_date=shift_datetime(unit="months", amount=3),
    )
    apiuser = UserModel(
        username=seed_apiuser_username,
        email="apiuser1@genassist.ritech.io",
        is_active=1,
        hashed_password=pwd_context.hash(seed_apiuser_password),
        user_type_id=console_user_type.id,
        force_upd_pass_date=shift_datetime(unit="months", amount=3),
    )

    transcribe_operator_user = UserModel(
        id=UUID(seed_test_data.transcribe_operator_user_id),
        username=seed_transcribe_operator_username,
        email="transcribeoperator1@genassist.ritech.io",
        is_active=1,
        hashed_password=pwd_context.hash(seed_transcribe_operator_password),
        user_type_id=console_user_type.id,
        force_upd_pass_date=shift_datetime(unit="months", amount=3),
    )
    ...
    transcribe_operator_user.user_roles.append(UserRoleModel(role=operator_role))
...
async def seed_connection_data_for_zendesk(
    ...
    try:
        ...
        if missing_values:
            raise ValueError(f"Missing required Zendesk settings: {list(missing_values.keys())}")
...
async def seed_connection_data_for_whatsapp(
    ...
    whatsapp_settings = {"whatsapp_token": settings.WHATSAPP_TOKEN}

    try:
        # Check if all required values are present
        missing_values = {k: v for k, v in whatsapp_settings.items() if not v}
        if missing_values:
            raise ValueError(f"Missing required WhatsApp settings: {list(missing_values.keys())}")
...
async def seed_connection_data_for_gmail(
    ...
    try:
        ...
        if missing_values:
            raise ValueError(f"Missing required Gmail settings: {list(missing_values.keys())}")
...
async def seed_connection_data_for_microsoft(
    ...
    try:
        ...
        if missing_values:
            raise ValueError(f"Missing required Microsoft settings: {list(missing_values.keys())}")
...
async def seed_connection_data_for_slack(
    ...
    slack_settings = {"slack_bot_token": settings.SLACK_TOKEN, "slack_signing_secret": settings.SLACK_SIGNING_SECRET}

    try:
        # Check if all required values are present
        missing_values = {k: v for k, v in slack_settings.items() if not v}
        if missing_values:
            raise ValueError(f"Missing required Slack settings: {list(missing_values.keys())}")
