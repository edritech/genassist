...
class TenantMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        logger.info(f"Request processed with tenant_slug: {tenant_slug}, tenant_id: {tenant_slug}")

        try:
            response = await call_next(request)
            return response
        except Exception as e:
            logger.error(f"Request failed for tenant {tenant_slug} (id: {tenant_slug}): {e}")
            raise
