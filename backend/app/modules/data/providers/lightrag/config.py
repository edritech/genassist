...
class LightRAGConfig(BaseModel):
    ...
    enabled: bool = Field(default=False, description="Whether LightRAG is enabled")

    # Core settings
    working_dir: str = Field(
        default=LIGHTRAG_DEFAULTS["working_directory"], description="Working directory for LightRAG data"
    )
    ...
    search_mode: str = Field(default=LIGHTRAG_DEFAULTS["search_mode"], description="Search mode (local, global, mix)")

    # Chunking settings
    chunk_token_size: int = Field(default=LIGHTRAG_DEFAULTS["chunk_token_size"], description="Chunk token size")
    chunk_overlap_token_size: int = Field(
        default=LIGHTRAG_DEFAULTS["chunk_overlap_token_size"], description="Chunk overlap token size"
    )

    # Vector storage settings
    vector_storage: str = Field(default=LIGHTRAG_DEFAULTS["vector_storage"], description="Vector storage type")
    ...
    # Query parameters
    top_k: int = Field(default=LIGHTRAG_DEFAULTS["top_k"], description="Top K results to return")
    response_type: str = Field(default=LIGHTRAG_DEFAULTS["response_type"], description="Response type for queries")
