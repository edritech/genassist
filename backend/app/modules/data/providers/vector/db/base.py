...
class VectorDBConfig(BaseModel):
    ...
    collection_name: str = Field(
        default_factory=lambda: VECTOR_DEFAULTS["vector_db_collection_name"] or "default",
        description="Name of the vector collection",
    )
    persist_directory: Optional[str] = Field(default=None, description="Directory for data persistence")
    host: Optional[str] = Field(default=settings.CHROMA_HOST, description="Database host")
    port: Optional[int] = Field(default=settings.CHROMA_PORT, description="Database port")
    ...
    hnsw_ef_search: int = Field(default=100, description="HNSW ef_search parameter")
...
class SearchResult(BaseModel):
    ...
    distance: Optional[float] = Field(default=None, description="Distance from query vector")
    ...
    def __init__(self, **data):
        ...
        if (
            "distance" in data
            and data.get("distance") is not None
            and ("score" not in data or data.get("score") is None)
        ):
