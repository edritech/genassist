...
class Office365Connector:
    ...
    def resolve_sharepoint_url(self, sharepoint_url: str):
        ...
        drives_resp = requests.get(f"https://graph.microsoft.com/v1.0/sites/{self.site_id}/drives", headers=headers)
    ...
    async def create_calendar_event(
        ...
        payload = {
            ...
            "attendees": [
                {"emailAddress": {"address": mail, "name": mail}, "type": "required"} for mail in (attendees or [])
            ],
    ...
    async def list_calendar_events(
        ...
        *,
        ...
        if attendee:
            filters.append(f"attendees/any(a:a/emailAddress/address eq '{attendee.lower()}')")
        if filters:
