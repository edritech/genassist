...
def create_tool_execution_info(
    iteration: int, tool_name: str, tool_input: Dict[str, Any], result: str, reasoning: str = ""
) -> Dict[str, Any]:
...
def create_error_response(error_msg: str, agent_type: str, **kwargs) -> Dict[str, Any]:
    ...
    response = {"status": "error", "error": error_msg, "agent_type": agent_type}
...
def create_success_response(result: str, agent_type: str, **kwargs) -> Dict[str, Any]:
    ...
    response = {"status": "success", "response": result, "agent_type": agent_type}
    response.update(kwargs)
    return response


def handle_parameter_validation_error(
    error: ValueError, tool_name: str, tool: BaseTool, agent_type: str, **kwargs
) -> Dict[str, Any]:
...
def get_tool_parameter_info(tool_map: Dict[str, BaseTool], tool_name: str) -> Dict[str, Any]:
    ...
    if tool_name not in tool_map:
        return {"status": "error", "error": f"Tool '{tool_name}' not found", "available_tools": list(tool_map.keys())}
