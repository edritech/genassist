    ...
    async def compact_messages(
        ...
        try:
            ...
            prompt = self.COMPACTION_PROMPT.format(
                ...
                "the least amount of previous context."
                if existing_summary
                else "",
            )
            ...
            result["compacted_message_count"] = (
                len(messages) + existing_summary.get("compacted_message_count", 0)
                if existing_summary
                else len(messages)
            )
            ...
            logger.info(
                f"Compacted {len(messages)} messages into summary with {len(result.get('entities', []))} entities"
            )
