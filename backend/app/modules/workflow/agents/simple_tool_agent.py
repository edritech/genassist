...
class SimpleToolAgent(BaseToolAgent):
    ...
    def _create_prompt(self, query: str, chat_history: Optional[List[Dict[str, str]]] = None) -> str:
        ...
        enhanced_prompt = create_tool_agent_tools_available_prompt(self.system_prompt, tool_descriptions)
        ...
        try:
            ...
            if not parsed:
                return create_error_response(
                    "LLM did not return a valid response", self._get_agent_name(), llm_response=response_content
                )
            ...
            if parsed.get("action") != "tool_call":
                return create_error_response(
                    "LLM did not return a valid tool_call action", self._get_agent_name(), llm_response=response_content
                )
            ...
            if not tool:
                ...
                direct_response = await self.llm_model.ainvoke([{"role": "user", "content": no_tools_prompt}])
                direct_content = self._extract_response_content(direct_response)
                direct_parsed = parse_json_response(direct_content)
                answer = direct_parsed.get("response", direct_content) if direct_parsed else direct_content
                return create_success_response(
                    answer,
                    self._get_agent_name(),
                    no_tool_used=True,
            ...
            logger.debug(f"SimpleToolAgent: Invoked {tool.name} with {validated_params}, result: {result}")
            ...
            if hasattr(tool, "return_direct") and tool.return_direct:
                ...
                return create_success_response(
                    str(result), self._get_agent_name(), tool=tool.name, parameters=validated_params, return_direct=True
                )
            ...
            if isinstance(result, dict) and "result" in result:
                ...
                result = str(result)
            return create_success_response(result, self._get_agent_name(), tool=tool.name, parameters=validated_params)
