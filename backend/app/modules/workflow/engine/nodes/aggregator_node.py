...
class AggregatorNode(BaseNode):
    ...
    def check_if_requirement_satisfied(self) -> bool:
        ...
        logger.debug(f"Aggregator {self.node_id} has no ready sources yet")
        return False
    ...
    async def process(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not source_nodes:
            try:
                ...
                source_nodes = []
        if not source_nodes:
            logger.warning(f"No source nodes found for aggregator {self.node_id}")
        ...
        if required_sources:
            ...
            if missing_sources:
                logger.error(f"Required source nodes not found: {missing_sources}")
        ...
        self.set_node_input({"source_nodes": source_nodes, "aggregation_strategy": aggregation_strategy})
        ...
        logger.debug(f"Sync aggregator node {self.node_id} completed with {len(aggregated_outputs)} outputs")
        return result
    ...
    def _aggregate_immediately(self, source_nodes: List[str], require_all_inputs: bool = True) -> Dict[str, Any]:
        ...
        for source_id in source_nodes:
            ...
            if source_output is not None:
                ...
                logger.debug(f"Source node {source_id} output: {source_output}")
            else:
    ...
    def _apply_aggregation_strategy(
        self, aggregated_outputs: Dict[str, Any], aggregation_strategy: str
    ) -> Dict[str, Any]:
        ...
        if aggregation_strategy == "merge":
            ...
            logger.warning(f"Unknown aggregation strategy: {aggregation_strategy}, using merge")
            return self._merge_outputs(aggregated_outputs)
    ...
    def _concat_outputs(self, outputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"concatenated_output": concatenated, "source_outputs": outputs}
