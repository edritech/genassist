...
class RouterNode(BaseNode):
    ...
    async def process(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not all([first_value, compare_condition, second_value]):
            logger.warning(f"RouterNode {self.node_id} missing configuration values")
            route = "false"
        else:
            route = self._evaluate_condition(first_value, compare_condition, second_value)
    ...
    def _evaluate_condition(self, first_value: str, compare_condition: str, second_value: str) -> str:
        """
        ...
        if compare_condition not in self.COMPARE_OPTIONS:
            logger.warning(f"RouterNode {self.node_id} unsupported condition: {compare_condition}")
        ...
        try:
            if compare_condition == "equal":
                ...
                route = "true" if first_value.startswith(second_value) else "false"
            elif compare_condition == "not_starts_with":
                route = "true" if not first_value.startswith(second_value) else "false"
            elif compare_condition == "ends_with":
                route = "true" if first_value.endswith(second_value) else "false"
            elif compare_condition == "not_ends_with":
                route = "true" if not first_value.endswith(second_value) else "false"
            elif compare_condition == "regex":
                route = "true" if re.search(second_value, first_value) else "false"
            else:
                route = "false"

        except Exception as e:
            logger.error(f"RouterNode {self.node_id} error evaluating condition: {e}")
            route = "false"
