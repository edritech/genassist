...
class ApiKeysRepository:
    ...
    async def get_by_hashed_value(self, hashed_value: str) -> Optional[ApiKeyModel | None]:
        query = (
            select(ApiKeyModel)
            .where(ApiKeyModel.hashed_value == hashed_value)
            .options(
    ...
    async def _get_by_name(self, api_key_name: str):
        # Need index
        result = await self.db.execute(select(ApiKeyModel).where(ApiKeyModel.name == api_key_name))
        return result.scalars().first()
    ...
    async def get_all(self, api_keys_filter: ApiKeysFilter) -> list[ApiKeyModel]:
        ...
        if api_keys_filter.user_id:
            query = query.where(ApiKeyModel.user_id == api_keys_filter.user_id)
