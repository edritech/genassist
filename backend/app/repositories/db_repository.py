...
class DbRepository(Generic[OrmModelT]):
    ...
    # ───────────── internal helpers ─────────────
    def _apply_eager_options(self, stmt, eager: Sequence[str] | None):
        if eager:
            options: List[Load] = [selectinload(getattr(self.model, rel)) for rel in eager]
    ...
    # ───────────── READ methods ────────────────
    async def get_all(self, *, filter_obj=None, eager: Sequence[str] | None = None) -> List[OrmModelT]:
        """
    ...
    async def get_by_id(self, obj_id: UUID, *, eager: Sequence[str] | None = None) -> Optional[OrmModelT]:
        stmt = self._apply_eager_options(select(self.model), eager).where(self.model.id == obj_id)
        result = await self.db.execute(stmt)
        return result.scalars().first()

    async def get_by_ids(self, ids: List[UUID], *, eager: Sequence[str] | None = None) -> List[OrmModelT]:
        if not ids:
            return []
        stmt = self._apply_eager_options(select(self.model), eager).where(self.model.id.in_(ids))
        result = await self.db.execute(stmt)
        return result.scalars().all()
