...
class FineTuningRepository(DbRepository[FineTuningJobModel]):
    ...
    async def get_job_by_openai_id(self, openai_job_id: str) -> Optional[FineTuningJobModel]:
        ...
        query = (
            select(FineTuningJobModel)
            .where(FineTuningJobModel.openai_job_id == openai_job_id)
            .options(
    ...
    async def get_job_by_id(self, job_id: UUID) -> Optional[FineTuningJobModel]:
        ...
        query = (
            select(FineTuningJobModel)
            .where(FineTuningJobModel.id == job_id)
            .options(joinedload(FineTuningJobModel.training_file), joinedload(FineTuningJobModel.validation_file))
        )
        result = await self.db.execute(query)
        return result.scalars().first()
    ...
    async def list_jobs(self, status: Optional[JobStatus] = None) -> List[FineTuningJobModel]:
    ...
    async def get_active_jobs(self) -> List[FineTuningJobModel]:
        ...
        query = select(FineTuningJobModel).where(
            FineTuningJobModel.status.in_([JobStatus.VALIDATING_FILES, JobStatus.QUEUED, JobStatus.RUNNING])
        )
        result = await self.db.execute(query)
        return result.scalars().all()
    ...
    async def get_jobs_by_status(self, statuses: Optional[list[JobStatus]] = None) -> List[FineTuningJobModel]:
        ...
        if statuses:
            query = query.where(FineTuningJobModel.status.in_(statuses))
        result = await self.db.execute(query)
        return result.scalars().all()
    ...
    async def get_job_by_fine_tuned_model(self, fine_tuned_model: str) -> Optional[FineTuningJobModel]:
        ...
        query = select(FineTuningJobModel).where(FineTuningJobModel.fine_tuned_model == fine_tuned_model)
        result = await self.db.execute(query)
