...
class OperatorStatisticsRepository:
    ...
    async def update(self, operator_id: UUID, **kwargs):
        ...
        result = await self.db.execute(select(OperatorModel.statistics_id).where(OperatorModel.id == operator_id))
        statistics_id = result.scalar_one_or_none()

        # Step 2: Update the statistics table
        await self.db.execute(
            update(OperatorStatisticsModel).where(OperatorStatisticsModel.id == statistics_id).values(**kwargs)
        )
        await self.db.commit()
