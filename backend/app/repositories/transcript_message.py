...
class TranscriptMessageRepository:
    ...
    async def get_latest_sequence_number(self, conversation_id: UUID) -> int:
    ...
    async def get_message_by_message_id(
        ...
        query = select(TranscriptMessageModel).where(TranscriptMessageModel.id == message_id)
    ...
    async def add_message_feedback(
        ...
        existing_feedback = await self.get_user_feedback_for_message(message_id, get_current_user_id())
    ...
    async def get_user_feedback_for_message(self, message_id: UUID, user_id: UUID) -> Optional[MessageFeedbackModel]:
        """Get a specific user's feedback for a message"""
        query = select(MessageFeedbackModel).where(
            MessageFeedbackModel.message_id == message_id, MessageFeedbackModel.feedback_user_id == user_id
        )
        result = await self.db.execute(query)
        return result.scalars().first()
    ...
    async def get_messages_by_type(self, conversation_id: UUID, message_type: str) -> List[TranscriptMessageModel]:
        """Get messages filtered by type"""
        query = (
            select(TranscriptMessageModel)
            .where(
                TranscriptMessageModel.conversation_id == conversation_id, TranscriptMessageModel.type == message_type
            )
            .order_by(TranscriptMessageModel.sequence_number)
        )
