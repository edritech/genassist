...
class MCPServerWorkflowResponse(MCPServerWorkflowBase):
    ...
    input_schema: Optional[Dict[str, Any]] = Field(
        None, description="Input schema extracted from chatInputNode (JSON Schema format)"
    )
    created_at: datetime
    updated_at: datetime
...
class MCPServerCreate(BaseModel):
    ...
    workflows: List[MCPServerWorkflowCreate] = Field(
        ..., min_length=1, description="List of workflows to expose as tools"
    )
    description: Optional[str] = None
    is_active: int = Field(default=1, ge=0, le=1)
...
class MCPServerUpdate(BaseModel):
    ...
    api_key: Optional[str] = Field(
        None, min_length=32, description="API key for authentication (minimum 32 characters)"
    )
    ...
    def validate_unique_tool_names(
        cls, v: Optional[List[MCPServerWorkflowCreate]]
    ) -> Optional[List[MCPServerWorkflowCreate]]:
        """Ensure tool names are unique within the workflows list"""
        if v is None:
            return v
        tool_names = [wf.tool_name for wf in v]
