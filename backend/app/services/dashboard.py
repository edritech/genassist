...
class DashboardService:
    ...
    async def get_summary_stats(
        ...
        return DashboardSummaryStats(
            active_agents=active_agents, workflow_runs=workflow_runs, avg_response_time_ms=avg_response_time
        )
    ...
    async def get_active_conversations(
        ...
        conversations = await self.dashboard_repo.get_active_conversations(
            limit=page_size, offset=offset, from_date=from_date, to_date=to_date
        )
    ...
    async def get_agents_stats(
        ...
        agent_stats = await self.dashboard_repo.get_agents_with_stats(from_date=from_date, to_date=to_date, limit=limit)
    ...
    async def get_integrations(self) -> IntegrationsResponse:
        ...
        integration_items = [
            IntegrationItem(
                ...
                or INTEGRATION_DESCRIPTIONS.get(integration.type, "Custom integration"),
    ...
    async def get_full_dashboard(
        self, days: int = 30, conversations_page: int = 1, conversations_page_size: int = 3, agents_limit: int = 5
    ) -> DashboardResponse:
        ...
        active_conversations = await self.get_active_conversations(
            page=conversations_page, page_size=conversations_page_size, from_date=from_date, to_date=to_date
        )
        agents = await self.get_agents_stats(from_date, to_date, limit=agents_limit)
        integrations = await self.get_integrations()

        return DashboardResponse(
            summary=summary, active_conversations=active_conversations, agents=agents, integrations=integrations
        )
