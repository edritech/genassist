...
class FeatureFlagService:
    ...
    async def get_all(self) -> List[FeatureFlagRead]:
        ...
        return [FeatureFlagRead.model_validate(r, from_attributes=True) for r in rows]

    async def get_by_id(self, id: UUID) -> FeatureFlagRead:
        row = await self.repo.get_by_id(id)
        if not row:
            raise AppException(status_code=404, error_key=ErrorKey.FEATURE_FLAG_NOT_FOUND)
