...
class FileManagerService:
    ...
    async def initialize(
        self, base_url: str, base_path: str, app_settings: Optional[AppSettingsRead] = None
    ) -> BaseStorageProvider:
        ...
        try:
            ...
            if app_settings:
                ...
                config["AWS_BUCKET_NAME"] = app_settings.values.get(
                    "aws_bucket_name", file_storage_settings.AWS_BUCKET_NAME
                )
            else:
    ...
    def build_file_headers(
        self, file: FileModel, content: Optional[bytes] = None, disposition_type: str = "inline"
    ) -> tuple[dict, str]:
        ...
        content_disposition = (
            f"{disposition_type};filename=\"{filename_encoded}\";filename*=UTF-8''{filename_utf8_encoded}"
        )
    ...
    async def create_file(
        ...
        try:
            ...
            file_storage_provider = (
                file_base.storage_provider or (self.storage_provider.name if self.storage_provider else None) or "local"
            )
            ...
            if file_content is not None:
                ...
                if not upload_result:
                    raise AppException(
                        ...
                        error_detail=f"Failed to upload file {file_path} on storage provider {self.storage_provider.name}",
                    )
        except Exception as e:
            logger.error(f"Failed to create file: {e}")
            raise AppException(
                error_key=ErrorKey.INTERNAL_ERROR,
                error_detail=f"Failed to create file {file_path} on storage provider {self.storage_provider.name}: {str(e)}",
            )
        finally:
    ...
    async def get_file_content(self, file: FileModel) -> bytes:
        ...
        download_path = (
            file.path if file.storage_provider == "s3" else f"{self.storage_provider.get_base_path()}/{file.path}"
        )
