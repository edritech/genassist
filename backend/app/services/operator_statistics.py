...
class OperatorStatisticsService:
    ...
    async def update_from_analysis(
        self, conversation_analysis: ConversationAnalysisRead, operator_id: UUID, conversation_duration: int
    ):
        # Update operator_statistics
        existing_stats = await self.get_by_operator_id(operator_id)
        if not existing_stats:
            existing_stats = await self.create(operator_id=operator_id)
        ...
        updated_avg_customer_satisfaction = (
            existing_stats.avg_customer_satisfaction * existing_stats.call_count
            + conversation_analysis.customer_satisfaction
        ) / new_call_count
        ...
        updated_avg_score = calculate_rating_score(
            positive_percentage=updated_avg_positive,
            negative_percentage=updated_avg_negative,
            neutral_percentage=updated_avg_neutral,
        )
