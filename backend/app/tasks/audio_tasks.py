...
async def transcribe_audio_files_async_with_scope(ds_id: Optional[str] = None):
    ...
    return await run_task_with_tenant_support(transcribe_audio_files_async, "S3 audio transcription", ds_id=ds_id)
...
async def transcribe_audio_files_async(ds_id: Optional[str] = None):
    ...
    for ds_item in dsList:
        ...
        audio_files = [f for f in result["files"] if f["key"].endswith((".mp3", ".m4a"))]
        ...
        for file_info in audio_files:
            try:
                ...
                if await audioService.recording_exists(file_info["key"], ds_item.id):
                    logger.info(f"Recording: {file_info['key']} of Datasource {ds_item.name} already processed!!!")
                ...
                logger.info(f"Transcribing file: {file_info['key']} of Datasource {ds_item.name}")
                # transcript = await transcribe_audio_whisper_no_save(upload_file)

                await audioService.process_recording(upload_file, metadata)

                logger.info(f"Transcription for {file_info['key']}: completed")

                # TODO: save it in Recording

                transcribed.append({"file": file_info["key"], "timestamp": datetime.now().isoformat()})
