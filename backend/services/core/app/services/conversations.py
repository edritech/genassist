...
class ConversationService:
    ...
    async def get_conversation_by_id(
        ...
        if not conversation and raise_not_found:
            raise AppException(
                ErrorKey.CONVERSATION_NOT_FOUND, status_code=404)
        return conversation
    ...
    async def get_conversations_by_customer_id(self, customer_id: UUID, raise_not_found: bool = True):
        ...
        if not conversations and raise_not_found:
            raise AppException(
                ErrorKey.CONVERSATIONS_NOT_FOUND, status_code=404)
    ...
    async def update_in_progress_conversation(
        ...
        new_messages = await self.save_new_messages(
            conversation_id,
            in_progress_conv_update.messages,
            next_sequence
        )
        ...
        agent_ratio, customer_ratio, total_word_count = calculate_incremental_word_counts(
            new_segment_inputs,
        ...
        incremental_duration = calculate_duration_from_transcript(
            new_segment_inputs)
        ...
        all_messages = await self.transcript_message_repo.get_messages_by_conversation_id(
            conversation_id,
        )
    ...
    async def save_new_messages(
        ...
        new_messages = [
            schema_to_transcript_message(
                segment, conversation_id, next_sequence + idx)
    ...
    async def finalize_in_progress_conversation(
        ...
        store_in_zendesk = os.getenv(
            "STORE_CONVERSATIONS_IN_ZENDESK", "false").lower() == "true"
        if store_in_zendesk:
    ...
    async def add_conversation_feedback(self, conversation_id: UUID, feedback: Feedback,
        ...
        existing_feedback = json.loads(
            conversation.feedback) if conversation.feedback else []
