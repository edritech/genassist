...
class Settings(BaseSettings):
    """Main settings class with all configuration fields"""
    ...
    core_service_timeout: float = Field(
        default=5.0,
...
def validate_settings() -> List[str]:
    ...
    if settings.multi_tenant_enabled and settings.max_tenant_id_length < 1:
        issues.append("Max tenant ID length must be at least 1")

    # Validate WebSocket settings
    if settings.max_connections_per_user < 1:
        issues.append("Max connections per user must be at least 1")

    if settings.websocket_message_size_limit < 1024:
