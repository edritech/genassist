...
class SocketConnectionManager:
    """
    Standalone WebSocket connection manager with Redis Pub/Sub support
    ...
    async def connect(
        ...
        # Use the WebSocket as-is without FastAPI context extraction
        # This simplified version doesn't need context capture
        logger.debug(f"[CONNECT] WebSocket type: {type(websocket)}")
        ...
        if self._redis_manager:
            try:
                ...
                await redis_client.publish(
                    redis_channel, json.dumps(message_data, default=str)
                )
                logger.info(
    ...
    async def _broadcast_local(
        ...
        logger.info(
            f"[BROADCAST_LOCAL] Current _rooms keys: {list(self._rooms.keys())}"
        )
        ...
        for conn in targets:
            if required_topic and required_topic not in conn.topics:
                continue
            try:
                # Simplified send without context management
                await conn.websocket.send_text(message)
                logger.debug(f"[BROADCAST_LOCAL] ✅ Sent to user {conn.user_id}")
...
_connection_manager: SocketConnectionManager = None


def get_connection_manager() -> SocketConnectionManager:
    """
    Get the global WebSocket connection manager instance.

    Returns:
        SocketConnectionManager: Global connection manager
    """
    global _connection_manager
    return _connection_manager


def set_connection_manager(manager: SocketConnectionManager) -> None:
    """
    Set the global WebSocket connection manager instance.

    Args:
        manager: Connection manager instance to set
    """
    global _connection_manager
    _connection_manager = manager


async def connect_websocket_to_manager(
    websocket: WebSocket,
    room_id,
    user_id: UUID,
...
) -> SocketConnectionManager | None:
    """
    Helper function to connect a WebSocket to the connection manager.

    This function handles the common pattern of connecting a WebSocket to a room
    in the connection manager, using the singleton instance from get_connection_manager().

    Args:
        websocket: The WebSocket connection to connect
        room_id: The room ID (UUID, Enum, or string)
    ...
    Returns:
        SocketConnectionManager instance if available, None otherwise

    Note:
        The connection manager's connect() method already calls websocket.accept(),
        so this function handles the websocket acceptance internally.
    """
    ...
    if connection_manager:
        ...
        # Fallback: accept websocket manually if connection manager is not available
        await websocket.accept()
        logger.warning(
            "Connection manager not available, using fallback websocket acceptance"
        )
