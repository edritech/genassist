...
from shared.schemas.api_key import ApiKeyBase
from shared.schemas.operator_auth import OperatorAuth
from shared.schemas.role import RoleRead
...
class UserTypeUpdate(BaseModel):
    name: Optional[str] = Field(
        ..., min_length=4, max_length=255, description="User type name"
    )
...
class UserReadAuth(UserRead):
    permissions: Optional[list[str]] = Field(
        [], description="Permissions needed for authorization"
    )
    operator: Optional[OperatorAuth] = Field(
        None, description="Operator needed for authorization"
    )
