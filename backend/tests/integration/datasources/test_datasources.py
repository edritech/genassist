...
async def test_create_datasource(client):
    ...
    # Create datasource
    response = client.post("/api/datasources/", headers={"X-API-Key": "test123"}, json=datasource_data)
...
async def test_get_datasource_by_id(client):
    ...
    response = client.get(f"/api/datasources/{datasource_id}", headers={"X-API-Key": "test123"})
    assert response.status_code == 200
...
async def test_get_all_datasources(client):
    ...
    # Get all datasources
    response = client.get("/api/datasources", headers={"X-API-Key": "test123"})
    assert response.status_code == 200
...
async def test_update_datasource(client):
    ...
    # Update data
    update_data = {"name": "Updated Datasource", "is_active": 0}

    # Update the datasource
    response = client.put(f"/api/datasources/{datasource_id}", headers={"X-API-Key": "test123"}, json=update_data)
    assert response.status_code == 200
...
async def test_delete_datasource(client):
    ...
    response = client.delete(f"/api/datasources/{datasource_id}", headers={"X-API-Key": "test123"})
    assert response.status_code == 200
