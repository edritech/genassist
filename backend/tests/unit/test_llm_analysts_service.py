...
def sample_llm_analyst_data():
    return {"name": "test_analyst", "llm_provider_id": uuid4(), "prompt": "Test prompt", "is_active": 1}


@pytest.mark.asyncio
async def test_create_success(
    llm_analyst_service, mock_repository, mock_llm_provider_repository, sample_llm_analyst_data
):
    # Setup
...
async def test_get_all_success(llm_analyst_service, mock_repository, sample_llm_analyst_data):
    # Setup
    mock_analysts = [LlmAnalystModel(id=uuid4(), **sample_llm_analyst_data) for _ in range(2)]
...
async def test_update_success(llm_analyst_service, mock_repository, sample_llm_analyst_data):
    # Setup
    analyst_id = uuid4()
    update_data = LlmAnalystUpdate(name="updated_analyst", prompt="Updated prompt")
    mock_analyst = LlmAnalystModel(id=analyst_id, **sample_llm_analyst_data)
    mock_repository.get_by_id.return_value = mock_analyst

    updated_data = {**sample_llm_analyst_data, **update_data.model_dump(exclude_unset=True), "id": analyst_id}
