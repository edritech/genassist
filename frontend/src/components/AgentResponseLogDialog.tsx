...
export function AgentResponseLogDialog({ isOpen, onOpenChange, messageId }: AgentResponseLogDialogProps) {
          ...
          {!loading && !error && data && (
            <>
              <div className="text-xs text-muted-foreground space-y-1">
                {data.id != null && (
                  <div>
                    <span className="font-medium">ID:</span> {String(data.id)}
                  </div>
                )}
                {data.conversation_id != null && (
                  <div>
                    <span className="font-medium">Conversation ID:</span> {String(data.conversation_id)}
                  </div>
                )}
                {data.transcript_message_id != null && (
                  <div>
                    <span className="font-medium">Message ID:</span> {String(data.transcript_message_id)}
                  </div>
                )}
                {data.logged_at != null && (
                  <div>
                    <span className="font-medium">Logged at:</span> {new Date(data.logged_at).toLocaleString()}
                  </div>
                )}
              </div>
              <div>
