...
export function ConnectionTestPanel({ isTesting, testStatus, onTest, disabled = false }: ConnectionTestPanelProps) {
  return (
      ...
      {isTesting ? (
        ...
        <Alert variant="destructive">
          ...
          <AlertDescription>
            ...
            {testStatus.message && (
              <span className="block mt-1 text-xs opacity-75 break-all">{testStatus.message}</span>
            )}
          </AlertDescription>
        </Alert>
      ) : (
        <Alert>
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>This configuration has not been tested. Verify it works before saving.</AlertDescription>
        </Alert>
      )}
      <Button type="button" className="w-full" onClick={onTest} disabled={isTesting || disabled}>
