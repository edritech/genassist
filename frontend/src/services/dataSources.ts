...
export const createDataSource = async (dataSourceData: DataSource): Promise<DataSource> => {
  try {
    const response = await apiRequest<DataSource>('POST', 'datasources/', JSON.parse(JSON.stringify(dataSourceData)));
    return response;
  } catch (error) {
...
export const updateDataSource = async (id: string, dataSourceData: Partial<DataSource>): Promise<DataSource> => {
  try {
...
export const getDataSourceFormSchemas = async (): Promise<DynamicFormSchema> => {
  try {
    return await apiRequest<DynamicFormSchema>('GET', '/datasources/form_schemas');
  } catch (error) {
...
export const getGmailClientId = async (): Promise<string> => {
  try {
    ...
    const gmailSetting = appSettings.find((setting) => setting.type === 'Gmail' && setting.is_active === 1);
...
export const createTempGmailDataSource = async (name: string, appSettingsId: string): Promise<string> => {
  try {
...
export const buildGmailOAuthUrl = (clientId: string, datasourceId: string): string => {
...
export const getOffice365SettingValue = async (setting_key_name: string): Promise<string> => {
  try {
    const appSettings = await getAllAppSettings();
    const microsoftSetting = appSettings.find((setting) => setting.type === 'Microsoft' && setting.is_active === 1);

    if (!microsoftSetting?.values?.[setting_key_name]) {
      throw new Error(`Microsoft ${setting_key_name} not found in app settings`);
    }
...
export const createTempOffice365DataSource = async (name: string, appSettingsId: string): Promise<string> => {
  try {
    const tempDataSource: Partial<DataSource> = {
      name,
      source_type: 'o365',
...
export function buildOffice365OAuthUrl(clientId: string, tenantId: string, dataSourceId: string): string {
  const redirectUri = encodeURIComponent(`${window.location.origin}/office365/oauth/callback`);
