...
import { BackendTranscript, TranscriptEntry } from '@/interfaces/transcript.interface';
...
export const conversationService = {
  ...
  fetchConversationsTranscriptsAndData: async (id: string): Promise<BackendTranscript> => {
    const response = await apiRequest<BackendTranscript>('get', `/conversations/${id}?include_feedback=true`);
    if (!response) {
  ...
  fetchTranscript: async (id: string): Promise<ConversationTranscript> => {
    const response = await apiRequest<BackendTranscript>('get', `/conversations/${id}`);
    ...
    const { neutral_sentiment, positive_sentiment, negative_sentiment } = analysis;

    if (positive_sentiment > neutral_sentiment && positive_sentiment > negative_sentiment) {
      dominantSentiment = 'positive';
    } else if (negative_sentiment > positive_sentiment && negative_sentiment > neutral_sentiment) {
  ...
  fetchActive: async (options?: {
    ...
    sentiment?: string;
    category?: string;
    hostility_neutral_max?: number | string;
    hostility_positive_max?: number | string;
    ...
    try {
      ...
      const normalizeApiList = (payload: unknown): BackendTranscript[] => {
        ...
        if (Array.isArray((asObj as { conversations?: unknown[] }).conversations))
          return (asObj as { conversations: unknown[] }).conversations as BackendTranscript[];
