...
export async function getFineTunableModels(): Promise<string[]> {
  const res = await apiRequest<string[] | PaginatedResponse<string>>('GET', 'openai/models/fine-tunable');
...
export async function getFineTuneJob(id: string, sync: boolean = true): Promise<FineTuneJob | null> {
  return apiRequest<FineTuneJob>('GET', `openai/fine-tuning/jobs/${id}${sync ? '?sync=True' : ''}`);
}

export async function createFineTuneJob(payload: CreateFineTuneJobRequest): Promise<FineTuneJob> {
  return apiRequest<FineTuneJob>('POST', 'openai/fine-tuning/jobs', JSON.parse(JSON.stringify(payload)));
}

export async function cancelFineTuneJob(id: string): Promise<{ status?: string } | null> {
  return apiRequest<{ status?: string }>('POST', `openai/fine-tuning/jobs/${id}/cancel`);
}

export async function uploadFineTuneFile(file: File, purpose: string = 'fine-tune'): Promise<OpenAIFileItem> {
...
export async function listOpenAIFiles(): Promise<OpenAIFileItem[]> {
  const res = await apiRequest<OpenAIFileItem[] | PaginatedResponse<OpenAIFileItem>>('GET', 'openai/files');
