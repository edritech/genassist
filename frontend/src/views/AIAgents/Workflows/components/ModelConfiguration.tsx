...
export const ModelConfiguration: React.FC<ModelConfigurationProps> = ({
  ...
  const handleMemoryTrimmingModeChange = (
    mode: 'message_count' | 'token_budget' | 'message_compacting' | 'rag_retrieval'
  ) => {
    onConfigChange({
      ...config,
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder={typeSelect === 'agent' ? 'e.g., Agent' : 'e.g., LLM Model'}
            className="w-full"
          />
        </div>
        <div className="space-y-2">
        ...
        {typeSelect && (
          <div className="flex flex-row gap-2 space-y-2 w-full justify-center items-center">
              ...
              >
                ...
                {typeSelect === 'agent' && (
                  <SelectContent>
                    ...
                    <SelectItem value="SimpleToolExecutor">SimpleToolExecutor</SelectItem>
            ...
            {typeSelect === 'agent' && (
              <div className="space-y-2">
                <Label htmlFor={`system-prompt-input-${id}`}>Max Iterations</Label>
                <Input
        ...
        <div className="space-y-2 flex items-center gap-2 w-full">
          ...
          <Switch id={`memory-switch-${id}`} checked={config.memory} onCheckedChange={handleMemoryChange} />
        </div>
        {config.memory && (
          <>
            <div className="space-y-2">
            ...
            {config.memoryTrimmingMode === 'message_count' || !config.memoryTrimmingMode ? (
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent className="max-w-xs text-balance">
                        Entities that must always be retained in the compaction summary (e.g. "client name", "project
                        ID")
                      </TooltipContent>
                    </Tooltip>
                  </div>
                  ...
                  {(config.compactingImportantEntities || []).length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {(config.compactingImportantEntities || []).map((entity) => (
                        <Badge key={entity} variant="secondary" className="flex items-center gap-1 pr-1">
                          {entity}
                          <button
                            type="button"
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent className="max-w-xs text-balance">
                        Below this total message count, all messages are passed verbatim to the LLM without any vector
                        operations.
                      </TooltipContent>
                    </Tooltip>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent className="max-w-xs text-balance">
                        Overlapping messages between consecutive groups, preserving context at group boundaries. Must be
                        less than group size.
                      </TooltipContent>
                    </Tooltip>
                  </div>
                  <Input
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent className="max-w-xs text-balance">
                        Most recent messages always included verbatim in context alongside the retrieved historical
                        groups.
                      </TooltipContent>
                    </Tooltip>
                  </div>
              ...
              <>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    <Label htmlFor={`conversation-history-tokens-${id}`}>
                      Conversation History Allocation (tokens)
                    </Label>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                      ...
                      <TooltipContent className="max-w-xs text-balance">estimate 0.75 words = 1 token</TooltipContent>
                    </Tooltip>
                  </div>
                  <Input
