...
import { Loader2, Send, ChevronDown, ChevronRight, RefreshCw, ClipboardList } from 'lucide-react';
...
const WorkflowTestDialog: React.FC<WorkflowTestDialogProps> = ({
  ...
  useEffect(() => {
    if (workflow && isOpen) {
      const chatInputNode = workflow.nodes.find((node) => node.type.includes('InputNode'));
      if (chatInputNode && chatInputNode.data.inputSchema) {
        ...
        Object.entries(chatInputNode.data.inputSchema).forEach(([key, field]: [string, SchemaField]) => {
          ...
          if (executionState?.session && typeof executionState.session === 'object') {
          ...
          initialInputs[key] = value !== undefined ? valueToString(value, field.type) : '';
        });
  ...
  const extractPauseInfo = (res: WorkflowTestResponse) => {
    ...
    const formSchema = ((output != null &&
      typeof output === 'object' &&
      (output as Record<string, unknown>).form_schema) ||
      res.form_schema ||
      res.state?.paused_form_schema) as PausedFormSchema | undefined;
    const nodeId = ((output != null && typeof output === 'object' && (output as Record<string, unknown>).node_id) ||
      res.node_id) as string | undefined;
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-7xl max-h-[95vh] w-full overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>Test your workflow configuration with sample inputs</DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-1 max-h-[calc(85vh-180px)] min-w-0">
          <div className="flex flex-col space-y-4 min-w-0 w-full">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="test-input-message">
                  Message
                  {inputSchema?.message?.required && <span className="text-red-500 ml-1">*</span>}
                </Label>
              ...
              <Input
                ...
                className={`flex-1 ${prefilledFields.has('message') ? 'border-blue-300 bg-blue-50' : ''}`}
              />
            </div>
                  ...
                  <div className="pl-4 space-y-4 border-l-2 border-gray-200">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <Label htmlFor="test-input-thread_id">
                          Thread ID
                          {inputSchema?.thread_id?.required && <span className="text-red-500 ml-1">*</span>}
                        </Label>
                      </div>
                    ...
                    {Object.entries(inputSchema)
                      .filter(([key, field]: [string, SchemaField]) => {
                        ...
                        return key !== 'message' && !field.stateful && key !== 'thread_id';
                      })
                            ...
                            {isBoolean ? (
                              <div className="flex items-center space-x-2">
                                <Checkbox
                                  ...
                                  disabled={testing || !!pausedFormSchema}
                                  className={prefilledFields.has(key) ? 'border-blue-300' : ''}
                                />
                                <Label htmlFor={`test-input-${key}`} className="text-sm font-normal cursor-pointer">
                                  {testInput[key] === 'true' || testInput[key] === '1' ? 'True' : 'False'}
                                </Label>
                              </div>
                            ) : isObjectOrArray ? (
                              <Textarea
                                ...
                                className={`flex-1 font-mono text-sm ${
                                  prefilledFields.has(key) ? 'border-blue-300 bg-blue-50' : ''
                                }`}
                                rows={4}
                              />
                            ) : (
                              <Input
                                ...
                                disabled={testing || !!pausedFormSchema}
                                className={`flex-1 ${prefilledFields.has(key) ? 'border-blue-300 bg-blue-50' : ''}`}
                              />
                            )}
                          </div>
                        );
              ...
              <div className="space-y-4 p-4 border-2 border-blue-200 rounded-lg bg-blue-50/50">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-blue-700">Workflow Paused — User Input Required</span>
                  </div>
                  <Button variant="ghost" size="sm" className="text-xs text-gray-500" onClick={handleStartOver}>
                    <RefreshCw className="h-3 w-3 mr-1" />
                    Start Over
                  </Button>
                </div>

                {pausedFormSchema.message && <p className="text-sm text-gray-600">{pausedFormSchema.message}</p>}

                <div className="space-y-3">
                  {pausedFormSchema.fields.map((field) => {
                    const fieldKey = `paused-${field.name}`;
                    const val = humanInTheLoopFormData[field.name] || '';
                    const onChange = (v: string) => setHumanInTheLoopFormData((prev) => ({ ...prev, [field.name]: v }));
                    ...
                    return (
                      <div key={field.name} className="space-y-1">
                        ...
                        {field.type === 'boolean' ? (
                          ...
                          <Select value={val} onValueChange={onChange} disabled={testing}>
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    disabled={
                      testing || pausedFormSchema.fields.some((f) => f.required && !humanInTheLoopFormData[f.name])
                    }
                    className="flex items-center gap-2"
                  >
                    {testing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
            ...
            {response && (
              <div className="space-y-2">
                <Label>Response {response?.status === 'success' ? '✅' : '❌'}</Label>
                ...
                {response?.status === 'success' && (
                  <div className="border rounded-md overflow-hidden">
                    <div className="max-h-80 overflow-y-auto p-2 space-y-3">
                      ...
                      >
                        <div className="font-semibold mb-1 text-xs uppercase text-blue-600">You</div>
                        <div className="whitespace-pre-wrap">
                          {typeof response.input === 'object' && response.input !== null
                      ...
                      {!isDebugMode ? (
                        <div className={`p-3 rounded-md border bg-gray-50 border-gray-100`}>
                          <div className={`font-semibold mb-1 text-xs uppercase text-green-600`}>Response</div>
                          {/* Explicitly surface SQL node parameters if present */}
                          {typeof response.output === 'object' &&
                            response.output &&
                            (response.output as Record<string, unknown>).parameters && (
                              <div className="mb-3 p-2 bg-white border rounded">
                                <div className="text-xs font-semibold mb-1">Parameters</div>
                                <div className="text-xs text-gray-700">
                                  datasource_id:{' '}
                                  {(((response.output as Record<string, unknown>).parameters as Record<string, unknown>)
                                ...
                                <pre className="text-xs bg-gray-50 p-2 rounded overflow-auto mt-1">
                                  {JSON.stringify(
                                    ((
                                      (response.output as Record<string, unknown>).parameters as Record<string, unknown>
                                    )?.node_parameters as Record<string, unknown>) || {},
                                    null,
                                    2
                                  )}
                                </pre>
                          ...
                          {typeof response.output === 'string' ? (
                            <div className="whitespace-pre-wrap">{response.output}</div>
                          ) : (
                            <JsonViewer
                        ...
                        <div className={`p-3 rounded-md border bg-gray-50 border-gray-100`}>
                          <div className={`font-semibold mb-1 text-xs uppercase text-green-600`}>Debug View</div>
                          <JsonViewer
                            data={response}
                            onCopy={(data) => {
                              navigator.clipboard.writeText(JSON.stringify(data, null, 2));
                ...
                {response.workflow_id && (
                  <div className="mt-2 text-xs text-gray-500">Workflow ID: {response.workflow_id}</div>
                )}
              </div>
            )}
          </div>
