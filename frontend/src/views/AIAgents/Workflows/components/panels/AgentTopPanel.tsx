...
const AgentTopPanel = ({ data, onUpdated }: { data?: AgentConfig; onUpdated?: () => void }) => {
  ...
  return (
    <>
      ...
      >
        <div className="text-sm font-bold text-gray-900 truncate w-full text-center">{data?.name}</div>
        <div className="text-xs font-normal text-gray-500 truncate w-full text-center">{data?.description}</div>
      </div>
      <AgentFormDialog
        ...
        data={{
          ...
          thinking_phrase_delay: data?.thinking_phrase_delay,
          possible_queries: data?.possible_queries,
          thinking_phrases: data?.thinking_phrases,
        }}
      />
    </>
  );
