...
export const APIToolDialog: React.FC<BaseNodeDialogProps<APIToolNodeData, APIToolNodeData>> = (props) => {
  ...
  useEffect(() => {
    ...
    setRequestBody(typeof data.requestBody === 'string' ? data.requestBody : JSON.stringify(data.requestBody) || '');
  }, [isOpen]);
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <DraggableInput
          ...
          className="break-all w-full"
        />
        <div className="text-xs text-gray-500 break-words">Use {'{{field}}'} to define dynamic parameters</div>
      </div>
          ...
          {Object.entries(headers).map(([key, value], idx) => (
            <div key={`header-${idx}`} className="flex items-center gap-2 w-full">
              ...
              <DraggableInput
                ...
                className="flex-1 text-xs min-w-0 w-full"
              />
              <Button size="icon" variant="ghost" className="h-6 w-6 flex-shrink-0" onClick={() => removeHeader(key)}>
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center">
          ...
          <Button size="sm" variant="outline" className="h-6 text-xs" onClick={addParameter}>
        ...
        <div className="space-y-2">
          {Object.entries(parameters).map(([key, value], idx) => (
            <div key={`param-${idx}`} className="flex items-center gap-2 w-full">
              <DraggableInput
