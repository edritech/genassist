...
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/RadixTooltip';
import { NodeConfigPanel } from '../components/NodeConfigPanel';
import { DraggableInput } from '../components/custom/DraggableInput';
import { useWorkflowExecution } from '../context/WorkflowExecutionContext';
import { BaseNodeDialogProps } from './base';

const AGGREGATION_STRATEGY_OPTIONS = ['list', 'merge', 'first', 'last'] as const;

type AggregationStrategyType = (typeof AGGREGATION_STRATEGY_OPTIONS)[number];

type AggregatorDialogProps = BaseNodeDialogProps<AggregatorNodeData, AggregatorNodeData>;
...
export const AggregatorDialog: React.FC<AggregatorDialogProps> = (props) => {
  ...
  const [aggregationStrategy, setAggregationStrategy] = useState<AggregationStrategyType>('list');
  ...
  useEffect(() => {
    if (isOpen) {
      setName(data.name || '');
      setAggregationStrategy((data.aggregationStrategy as AggregationStrategyType) ?? 'list');
      ...
      const directSources = edges?.filter((e) => e.target === props.nodeId)?.map((e) => e.source) ?? [];

      const autoTemplate = directSources.length ? directSources.map((id) => `{{node_outputs.${id}}}`).join(', ') : '';
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <Select
          ...
          onValueChange={(value) => setAggregationStrategy(value as AggregationStrategyType)}
        >
        ...
        <p className="text-sm text-muted-foreground">
          ...
          {aggregationStrategy === 'merge' && 'Merge all results into a single object'}
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          className="w-full"
        />
        <p className="text-sm text-muted-foreground">Maximum time to wait for all inputs before proceeding</p>
      </div>
      ...
      <div className="space-y-2">
        ...
        <DraggableInput
          ...
          className="w-full"
        />
        <p className="text-sm text-muted-foreground">Template for forwarding aggregated results to downstream nodes</p>
      </div>
        ...
        <div className="flex items-center gap-2 w-full !mt-6">
          ...
          <Tooltip>
            ...
            <TooltipContent className="max-w-xs text-balance">
              When enabled, all connected branches must complete before results are merged. When disabled, available
              results are merged immediately without waiting for remaining branches.
            </TooltipContent>
          </Tooltip>
          <div className="flex-1" />
          <Switch id="require-all-inputs" checked={requireAllInputs} onCheckedChange={setRequireAllInputs} />
        </div>
      </TooltipProvider>
    </NodeConfigPanel>
  );
