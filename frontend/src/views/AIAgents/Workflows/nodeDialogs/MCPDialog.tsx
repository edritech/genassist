...
export const MCPDialog: React.FC<MCPDialogProps> = (props) => {
  ...
  const [connectionType, setConnectionType] = useState<MCPConnectionType>(getInitialConnectionType());

  const [connectionConfig, setConnectionConfig] = useState<STDIOConnectionConfig | HTTPConnectionConfig>(
    getInitialConnectionConfig()
  );

  const [availableTools, setAvailableTools] = useState<MCPTool[]>(data.availableTools || []);
  const [whitelistedTools, setWhitelistedTools] = useState<string[]>(data.whitelistedTools || []);
  const [isLoadingTools, setIsLoadingTools] = useState(false);

  // STDIO-specific state
  const [stdioCommand, setStdioCommand] = useState(
    connectionType === 'stdio' && 'command' in connectionConfig ? connectionConfig.command : ''
  );
  const [stdioArgs, setStdioArgs] = useState(
    connectionType === 'stdio' && 'args' in connectionConfig ? connectionConfig.args?.join(', ') || '' : ''
  );
  const [stdioEnv, setStdioEnv] = useState(
    connectionType === 'stdio' && 'env' in connectionConfig ? JSON.stringify(connectionConfig.env || {}, null, 2) : '{}'
  );

  // HTTP/SSE-specific state
  const [httpUrl, setHttpUrl] = useState(
    (connectionType === 'http' || connectionType === 'sse') && 'url' in connectionConfig ? connectionConfig.url : ''
  );
  ...
  const handleDiscoverTools = async () => {
    if (connectionType === 'stdio') {
      toast.error(
        'Tool discovery is not available for STDIO connections. Tools will be discovered automatically when the workflow runs.'
      );
      return;
    }
    ...
    try {
      ...
      const tools = await discoverMCPTools(connectionType, connectionConfig as HTTPConnectionConfig);
      setAvailableTools(tools);

      // Keep only whitelisted tools that still exist in the new list
      setWhitelistedTools((prev) => prev.filter((toolName) => tools.some((tool) => tool.name === toolName)));
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <Select value={connectionType} onValueChange={(value) => setConnectionType(value as MCPConnectionType)}>
        ...
        <>
          ...
          <div className="space-y-2">
            ...
            <DraggableInput
              ...
              className="w-full"
            />
            <div className="text-xs text-gray-500 break-words">API key for authenticating with the MCP server</div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              min="1"
            />
            <div className="text-xs text-gray-500 break-words">Request timeout in seconds (default: 30)</div>
          </div>
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center">
          ...
          >
            <RefreshCw className={`h-3 w-3 mr-1 ${isLoadingTools ? 'animate-spin' : ''}`} />
            Discover Tools
          </Button>
        </div>
        {connectionType === 'stdio' && (
        ...
        {availableTools.length > 0 && (
          <div className="flex gap-2 mb-2">
            <Button size="sm" variant="outline" onClick={selectAllTools} className="text-xs">
              Select All
            </Button>
            <Button size="sm" variant="outline" onClick={deselectAllTools} className="text-xs">
              Deselect All
            </Button>
          </div>
        )}
          ...
          {availableTools.length === 0 ? (
            ...
            <div className="space-y-2">
              {availableTools.map((tool) => (
                <div key={tool.name} className="flex items-start space-x-2 w-full p-2 hover:bg-gray-50 rounded">
                  <Checkbox
                    id={`tool-${tool.name}`}
                  ...
                  <div className="flex-1 min-w-0">
                    <Label htmlFor={`tool-${tool.name}`} className="text-sm font-medium cursor-pointer block">
                      {tool.name}
                    </Label>
                    {tool.description && <p className="text-xs text-gray-500 mt-1 break-words">{tool.description}</p>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <p className="text-xs text-gray-500 break-words">
          Select which tools to expose to your agent. Only whitelisted tools will be available for the agent to use.
        </p>
      </div>
    </NodeConfigPanel>
  );
