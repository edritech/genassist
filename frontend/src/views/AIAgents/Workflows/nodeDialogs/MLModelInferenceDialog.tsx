...
> = (props) => {
  ...
  const [inferenceInputs, setInferenceInputs] = useState<Record<string, string>>(data.inferenceInputs || {});
    ...
    >
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={modelId} onValueChange={handleModelChange} disabled={loading}>
            <SelectTrigger>
              <SelectValue placeholder={loading ? 'Loading models...' : 'Select an ML model'} />
            </SelectTrigger>
            <SelectContent>
              {mlModels.map((model) => (
          ...
          {selectedModel && <div className="text-xs text-gray-500">{selectedModel.description}</div>}
        </div>

        {/* Inference Parameters */}
        {selectedModel && selectedModel.features && selectedModel.features.length > 0 && (
          <div className="space-y-2">
            <Label className="text-sm font-semibold">Inference Values</Label>
            <div className="space-y-3 pl-2 border-l-2 border-gray-200">
              {selectedModel.features.map((key) => (
                <div key={key} className="space-y-1">
                  <Label htmlFor={`param-${key}`} className="text-xs text-gray-600">
                    {key}
                  </Label>
                  <DraggableInput
                    id={`param-${key}`}
                    value={inferenceInputs[key] || ''}
                    onChange={(e) => updateInferenceInput(key, e.target.value)}
