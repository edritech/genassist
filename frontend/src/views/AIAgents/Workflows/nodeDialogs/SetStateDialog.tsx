...
type SetStateDialogProps = BaseNodeDialogProps<SetStateNodeData, SetStateNodeData>;
    ...
    <NodeConfigPanel
      footer={
        <>
          ...
          <Button onClick={handleSave} disabled={states.length === 0 || states.every((s) => !s.key.trim())}>
            ...
            <HoverCard>
              ...
              <HoverCardContent className="w-80">
                <div className="space-y-2">
                  <p className="text-sm font-medium">State Entries Guidelines:</p>
                  <ul className="text-xs space-y-1">
                    <li>• Add multiple state entries to update several parameters at once</li>
                    <li>• Select stateful parameters from chat input nodes</li>
                    <li>• Use {'{{variable}}'} syntax for dynamic values</li>
                    <li>• Values will persist across workflow executions</li>
                  </ul>
                </div>
              </HoverCardContent>
            </HoverCard>
          </div>
          <Button type="button" variant="outline" size="sm" onClick={addStateEntry} className="flex items-center gap-1">
        ...
        <div className="space-y-4">
          {states.map((state, index) => (
            <div key={index} className="p-4 border border-gray-200 rounded-md space-y-3 bg-gray-50">
              <div className="flex items-center justify-between">
                <Label className="text-sm font-medium">State Entry {index + 1}</Label>
                {states.length > 1 && (
                  <Button
                    type="button"
                    variant="ghost"
              ...
              <div className="space-y-2">
                ...
                {statefulParams.length > 0 ? (
                  <Select value={state.key} onValueChange={(value) => updateStateEntry(index, 'key', value)}>
                  ...
                  <>
                    <Input
                      value={state.key}
                      onChange={(e) => updateStateEntry(index, 'key', e.target.value)}
              ...
              <div className="space-y-2">
                ...
                <DraggableInput
                  ...
                  value={state.value}
                  onChange={(e) => updateStateEntry(index, 'value', e.target.value)}
                  placeholder="Enter value or drag variable from JSON viewer"
                  className="w-full"
                />
                <div className="text-xs text-gray-400">Use {'{{variable}}'} for dynamic values</div>
              </div>
            </div>
          ))}
        </div>
