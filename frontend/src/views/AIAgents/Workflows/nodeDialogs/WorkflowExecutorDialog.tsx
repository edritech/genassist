...
> = (props) => {
  ...
  const [inputParameters, setInputParameters] = useState<Record<string, string>>(data.inputParameters || {});
  ...
  const extractInputSchema = (workflow: Workflow) => {
    ...
    const chatInputNode = workflow.nodes.find(
      (node) => node.type?.includes('InputNode') || node.type === 'chatInputNode'
    );
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2">
        ...
        <Select value={workflowId} onValueChange={handleWorkflowChange} disabled={loading}>
          <SelectTrigger>
        ...
        {workflowName && <p className="text-xs text-gray-500">Selected: {workflowName}</p>}
      </div>
          ...
          <div className="space-y-3 pl-2 border-l-2 border-gray-200">
            {Object.entries(selectedWorkflowInputSchema).map(([key, field]: [string, SchemaField]) => (
              <div key={key} className="space-y-1">
                <Label htmlFor={`param-${key}`} className="text-xs text-gray-600 flex items-center gap-1">
                  {key}
                  {field.required && <span className="text-red-500">*</span>}
                  {field.description && <span className="text-gray-400 font-normal">({field.description})</span>}
                </Label>
                <DraggableInput
                  id={`param-${key}`}
      ...
      {workflowId && selectedWorkflowInputSchema && Object.keys(selectedWorkflowInputSchema).length === 0 && (
        <div className="text-sm text-gray-500 text-center py-4">This workflow has no input parameters defined.</div>
      )}

      {!workflowId && (
        <div className="text-sm text-gray-500 text-center py-4">Select a workflow to configure input parameters.</div>
      )}
    </NodeConfigPanel>
  );
};
