...
export const ICON_MAPPING: Record<string, IconConfig> = {
  ...
  SplitRotated: {
    type: 'lucide',
    source: (props) => <Split {...props} style={{ transform: 'rotate(90deg)' }} />,
  },
  Merge: { type: 'lucide', source: Merge },
  MergeRotated: {
    type: 'lucide',
    source: (props) => <Merge {...props} style={{ transform: 'rotate(90deg)' }} />,
  },
...
export const renderIcon = (iconName: string, className: string = 'h-4 w-4 text-white', style?: React.CSSProperties) => {
  ...
  if (iconConfig.type === 'lucide') {
    ...
    const isPanelIcon = className.includes('text-') && !className.includes('text-white');
