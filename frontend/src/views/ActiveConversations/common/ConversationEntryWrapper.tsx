    ...
    if (entry.type === 'file') {
      ...
      return (
        <div className="bg-blue-50 border border-blue-200 rounded-md p-3 max-w-sm">
          ...
          <p className="text-xs text-blue-700 mb-2">{formSchema?.message || 'User input requested'}</p>
          <div className="flex flex-wrap gap-1">
            {formSchema?.fields?.map((f: { label: string; type: string; required?: boolean }, i: number) => (
              <span
                key={i}
                className="inline-flex items-center rounded border border-blue-300 bg-white px-1.5 py-0.5 text-[10px] text-blue-800"
              >
                {f.label}
