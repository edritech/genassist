...
import { Transcript, TranscriptEntry, ConversationFeedbackEntry } from '@/interfaces/transcript.interface';
...
function TranscriptDialogContent({
  ...
  const feedbackCacheRef = useRef<Map<string, ConversationFeedbackEntry>>(new Map());
  ...
  const hasSupervisorTakeover = useMemo(() => {
    ...
    return transcript.status === 'takeover' || transcriptMessages.some((entry) => entry.type === 'takeover');
  ...
  const [localMessages, setLocalMessages] = useState<TranscriptEntry[]>(() => transcriptMessages);
    ...
    if (statistics && transcript?.id) {
      setConversationStats((prevStats) => {
        ...
        if (
          ...
          newStats.in_progress_hostility_score !== statistics.in_progress_hostility_score
        ) {
          newStats.in_progress_hostility_score = Number(statistics.in_progress_hostility_score);
          hasUpdates = true;
        }
  ...
  const feedbackCount = Array.isArray(transcript?.feedback) ? transcript.feedback.length : 0;
  useEffect(() => {
    if (!isOpen) return;
  ...
  useEffect(() => {
    ...
    if (wsMessages.length > 0) {
      for (const msg of wsMessages) {
        ...
        if (!currentMsgs.some((m) => m.text === msg.text && toEpochMs(m.create_time) === toEpochMs(msg.create_time))) {
    ...
    for (const sentMsg of sentMessages) {
      if (
        !currentMsgs.some((m) => m.text === sentMsg.text && toEpochMs(m.create_time) === toEpochMs(sentMsg.create_time))
      ) {
        currentMsgs.push(sentMsg);
      }
    }

    if (transcript?.status === 'takeover' && !currentMsgs.some((m) => m.type === 'takeover')) {
      const now = Date.now();
    ...
    if (!userInitiatedTakeOver) {
      setHasTakenOver(transcript?.status === 'takeover' || currentMsgs.some((msg) => msg.type === 'takeover'));
    }
  ...
  const liveTopic =
    typeof statistics?.topic === 'string' && statistics.topic.trim() !== '' ? statistics.topic : undefined;
  const topicText = liveTopic || conversationStats.topic || transcript.metadata?.topic || 'Active Conversation';
  ...
  const handleTakeOver = async () => {
    ...
    try {
      ...
      if (success) {
        ...
        try {
          if (userFeedback) {
            if (Array.isArray(transcript.feedback)) {
              ...
              if (!exists) transcript.feedback = [...transcript.feedback, userFeedback];
            } else {
        ...
        setLocalMessages((prev) => (prev.some((m) => m.type === 'takeover') ? prev : [...prev, takeoverEntry]));
  ...
  const durationInSeconds =
    conversationStats.duration > 3600 * 24 ? Math.floor(conversationStats.duration / 1000) : conversationStats.duration;
  ...
  const handleFeedbackSubmit = async () => {
    ...
    try {
      const ok = await submitConversationFeedback(transcript.id, feedbackType, feedbackMessage.trim());
      if (ok) {
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:h-[550px] md:overflow-hidden">
        <div className="space-y-4 flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto">
            {leftPanelTab === 'feedback' ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mb-2">
                        {new Date(userFeedback.feedback_timestamp).toLocaleString()}
                      </p>
              ...
              <div className="space-y-4">
                <InfoBox icon={<Timer />} label="Duration" value={formatDuration(durationInSeconds)} />
                <InfoBox
                  icon={<User />}
                  label="Agent/Customer Ratio"
                  value={`${conversationStats.agent_ratio || 0}% / ${conversationStats.customer_ratio || 0}%`}
                />
                <InfoBox icon={<MessageCircle />} label="Word Count" value={`${conversationStats.word_count || 0}`} />
                <InfoBox icon={<Frown />} label="Hostility" value={`${in_progress_hostility_score}%`} />

                <TopicBox text={hasTakenOver ? 'You have taken over this conversation' : topicText} />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="md:col-span-2 flex flex-col h-full min-h-0">
          <div ref={scrollRef} className="flex-1 flex flex-col bg-secondary/30 rounded-lg p-3 overflow-y-auto min-h-0">
            {localMessages.length > 0 ? (
              <div className="space-y-2">
                ...
                {localMessages.map((entry, idx) => {
                  ...
                  const isAgent = speaker.includes('agent') || speaker.includes('operator');
                  const isCustomer = speaker.includes('customer') || (!isAdmin && !isAgent);
                  const speakerName = isAdmin ? 'Admin' : isAgent ? 'Agent' : isCustomer ? 'Customer' : 'Unknown';
                  if (!entry.text || !entry.text.trim()) return null;

                  return (
                    <div key={`${transcript.id}-message-${idx}-${entry.create_time}`} className="message-container">
                      <div className={`flex flex-col ${isAgent ? 'items-end' : 'items-start'}`}>
                        ...
                        <div
                          className={`p-2 rounded-lg max-w-[75%] sm:max-w-[90%] leading-tight break-words ${
                            isAgent ? 'bg-blue-500 text-white rounded-tl-lg' : 'bg-gray-200 text-gray-900 rounded-tr-lg'
                          }`}
                        >
                ...
                {isThinking && !hasTakenOver && (
                  <div className="flex flex-col items-end">
                    <span className="text-[11px] text-black font-medium mb-1">Agent</span>
          ...
          <div className="mt-4 space-y-3">
            {!hasTakenOver ? (
              ...
              <>
                ...
                <Button onClick={handleFinalize} className="bg-red-600 text-white w-full" disabled={isFinalizing}>
...
function InfoBox({ icon, label, value }: { icon: JSX.Element; label: string; value: React.ReactNode }) {
  return (
