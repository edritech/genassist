...
export function IntegrationWorkflowsDialog({ integration, open, onOpenChange }: IntegrationWorkflowsDialogProps) {
      ...
      try {
        ...
        const filteredWorkflows = allWorkflows
          .map((workflow) => {
            ...
            const usageCount = nodes.filter((node) => nodeTypes.includes(node.type || '')).length;
        ...
        <DialogHeader>
          <div className="flex items-center gap-3">
            <div className={`${integration.bgColor} flex items-center p-3 rounded-lg`}>
              <div className={integration.iconColor}>{getIcon(integration.icon, 'w-6 h-6')}</div>
            </div>
            <div>
              <DialogTitle className="text-xl">{integration.name} Workflows</DialogTitle>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              <div className="mb-4 flex justify-center">
                <div className="bg-gray-100 p-4 rounded-full">{getIcon(integration.icon, 'w-8 h-8 text-gray-400')}</div>
              </div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No workflows yet</h3>
              <p className="text-gray-600 mb-6 max-w-sm mx-auto">
                Create your first workflow using {integration.name} to automate your processes.
              </p>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-green-100 text-green-700 rounded-md text-xs font-medium">
                          ...
                          <span>Used {workflow.usageCount}x</span>
                        </div>
