...
export function AppSettingDialog({
  ...
  const renderField = (field: FieldSchema) => {
    ...
    switch (field.type) {
      case 'select':
        return (
          <Select value={value as string} onValueChange={(val) => handleValuesChange(field, val)}>
  ...
  const requiredFields =
    type !== 'Other' && appSettingSchemas[type] ? appSettingSchemas[type].fields.filter((f) => f.required) : [];
