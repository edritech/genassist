...
export function AppSettingsCard({ searchQuery, appSettings, onEditSetting, onDeleteSetting }: AppSettingsCardProps) {
  ...
  const renderRow = (item: DisplayItem) => {
    ...
    rows.push(
      <TableRow key={setting.id}>
          ...
          <div className="flex flex-col gap-1 max-w-md">
            {Object.entries(setting.values || {}).map(([key, value]) => (
              <div key={key} className="text-sm">
                ...
                <span className="font-mono text-xs">{value && value.length > 0 ? '••••••••' : '—'}</span>
              </div>
            ))}
            {Object.keys(setting.values || {}).length === 0 && <span className="text-gray-400">—</span>}
          </div>
        </TableCell>
        ...
        <TableCell className="truncate">{setting.created_at ? formatDate(setting.created_at) : 'No date'}</TableCell>
        <TableCell>
          <ActionButtons
