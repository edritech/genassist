...
export function MCPServerDetailsDialog({ isOpen, onOpenChange, serverId, onEdit }: Props) {
          ...
          {server.url && (
            <div>
              ...
              <div className="mt-1 flex items-center gap-2">
                <code className="flex-1 bg-gray-100 px-3 py-2 rounded text-sm font-mono break-all">{server.url}</code>
                <Button
                  type="button"
                  variant="outline"
                  size="icon"
              ...
              <p className="text-xs text-gray-500 mt-1">Use this URL to connect MCP clients to this server</p>
            </div>
          )}
            ...
            <div className="mt-2 space-y-2">
              {server.workflows.length === 0 ? (
                ...
                server.workflows.map((workflow, index) => (
                  <div key={workflow.workflow_id} className="border rounded-md p-3 bg-gray-50">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <p className="text-sm font-medium">{workflow.tool_name}</p>
                        <p className="text-xs text-gray-500 mt-1">{workflow.tool_description}</p>
                      </div>
                    </div>
                  </div>
                ))
