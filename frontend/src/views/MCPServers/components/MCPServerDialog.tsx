...
export function MCPServerDialog({
  ...
  const handleSubmit = async () => {
    ...
    const invalidWorkflows = selectedWorkflows.filter((w) => !w.tool_name.trim() || !w.tool_description.trim());
  ...
  const updateWorkflowToolName = (workflowId: string, toolName: string) => {
    setSelectedWorkflows(
      selectedWorkflows.map((w) => (w.workflow_id === workflowId ? { ...w, tool_name: toolName } : w))
    );
  };

  const updateWorkflowToolDescription = (workflowId: string, toolDescription: string) => {
    setSelectedWorkflows(
      selectedWorkflows.map((w) => (w.workflow_id === workflowId ? { ...w, tool_description: toolDescription } : w))
    );
  };

  const availableWorkflows = workflows.filter((w) => !selectedWorkflows.some((sw) => sw.workflow_id === w.id));
    ...
    <Dialog open={isOpen} onOpenChange={handleDialogClose}>
      <DialogContent className="sm:max-w-[700px] p-0 flex flex-col max-h-[90vh]">
        <DialogHeader className="p-6 pb-4 flex-shrink-0">
          <DialogTitle>{mode === 'create' ? 'Add New MCP Server' : 'Edit MCP Server'}</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto px-6">
          <div className="grid gap-4 pb-4">
            <div>
              ...
              <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="My MCP Server" />
            </div>

            <div>
              <div className="flex items-center justify-between mb-2">
                <Label htmlFor="api-key">API Key {mode === 'create' ? '*' : '(leave blank to keep existing)'}</Label>
                {mode === 'create' && (
                  <Button type="button" variant="outline" size="sm" onClick={generateApiKey} className="h-7 text-xs">
                ...
                <div className="mb-3 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1">
                      <p className="text-xs font-medium text-yellow-800 mb-1">Important: Copy this API key now</p>
              ...
              <div className="flex items-center gap-2">
                ...
                {apiKey && (
                  <Button
                    ...
                    title="Copy API key"
                  >
                    {isApiKeyCopied ? <Check className="h-4 w-4 text-green-600" /> : <Copy className="h-4 w-4" />}
                  </Button>
                )}
              </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mb-3">
                Select workflows to expose as MCP tools. Each workflow will be available as a tool with a custom name
                and description.
              </p>
              ...
              {selectedWorkflows.length === 0 && availableWorkflows.length === 0 ? (
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                  {selectedWorkflows.map((sw) => {
                    ...
                    return (
                      <div key={sw.workflow_id} className="border rounded-lg p-4 space-y-3 bg-white shadow-sm">
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-sm truncate">{workflow?.name || 'Unknown Workflow'}</p>
                            {workflow?.description && (
                              <p className="text-xs text-gray-500 mt-1 line-clamp-1">{workflow.description}</p>
                            )}
                          </div>
                          <Button
                            type="button"
                        ...
                        <div className="grid grid-cols-1 gap-3">
                          <div>
                            ...
                            <Input
                              ...
                              value={sw.tool_name}
                              onChange={(e) => updateWorkflowToolName(sw.workflow_id, e.target.value)}
                          ...
                          <div>
                            ...
                            <Textarea
                              ...
                              onChange={(e) => updateWorkflowToolDescription(sw.workflow_id, e.target.value)}
                              placeholder="Description of what this tool does"
                              rows={2}
                              className="text-sm mt-1"
                            />
                            <p className="text-xs text-gray-500 mt-1">Description that will be shown to MCP clients</p>
                          </div>
                        </div>
                      </div>
                    );
                  ...
                  {availableWorkflows.length > 0 && (
                    <div className="relative">
                      <Select onValueChange={addWorkflow} disabled={isLoadingWorkflows}>
        ...
        <DialogFooter className="px-6 py-4 border-t flex-shrink-0">
          <div className="flex justify-end gap-3 w-full">
            ...
            <Button onClick={handleSubmit} disabled={isSubmitting}>
              {isSubmitting ? 'Saving...' : mode === 'create' ? 'Create MCP Server' : 'Update MCP Server'}
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
