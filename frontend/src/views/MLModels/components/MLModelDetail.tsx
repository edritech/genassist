...
import { TrainingPipelineConfig, PipelineRun, PipelineArtifact } from '@/interfaces/ml-model-pipeline.interface';
            ...
            <div className="max-w-7xl mx-auto">
              <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {model.pkl_file_id && (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => downloadModelFile(model.pkl_file_id as string)}
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {model.created_at && (
                      <div>
                        ...
                        <p className="text-sm font-medium">{new Date(model.created_at).toLocaleString()}</p>
                      </div>
                    )}
                    {model.updated_at && (
                      <div>
                        <Label className="text-sm text-gray-500 mb-1 block">Updated At</Label>
                        <p className="text-sm font-medium">{new Date(model.updated_at).toLocaleString()}</p>
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-6">
                              ...
                              <div className="flex items-start justify-between">
                                <div className="flex-1">
                                  <div className="flex items-center gap-2 mb-2">
                                    <h4 className="text-lg font-semibold">{workflow?.name || 'Unknown Workflow'}</h4>
                                    {config.is_default && (
                                  ...
                                  <div className="flex flex-wrap gap-4 text-xs text-gray-500">
                                    {workflow?.version && <span>Version: {workflow.version}</span>}
                                    {config.cron_schedule && (
                  ...
                  <div className="rounded-lg border bg-white p-6">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      {pipelineRuns.length > 0 && <Badge variant="outline">{pipelineRuns.length} runs</Badge>}
                    </div>
                          ...
                          return (
                            <div key={run.id} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                              <div className="flex items-center justify-between">
                                <div className="flex-1">
                                  <div className="flex items-center gap-2 mb-2">
                                    <h4 className="text-base font-semibold">{workflow?.name || 'Unknown Workflow'}</h4>
                                  ...
                                  {isSuccessful && run.execution_output && (
                                    <div className="mt-2 text-xs text-gray-500">Execution completed successfully</div>
                                  )}
                                </div>
                                <div className="flex gap-2 ml-4">
                                  <Button
                                  ...
                                  <Button variant="outline" size="sm" onClick={() => handleViewRunDetails(run)}>
                ...
                <Dialog open={showCreateWorkflowDialog} onOpenChange={setShowCreateWorkflowDialog}>
                  <DialogContent>
                    ...
                    <div className="space-y-4 py-4">
                      ...
                      <p className="text-sm text-gray-500">
                        After creating the workflow, you can configure it in the Workflow Studio by navigating to AI
                ...
                <ConfirmDialog
                  ...
                  itemName={
                    configToDelete?.workflow_id
                      ? workflows.find((w) => w.id === configToDelete.workflow_id)?.name || 'configuration'
                      : 'configuration'
                  }
