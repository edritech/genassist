      ...
      <PopoverTrigger asChild>
        <button
          className={`p-2 rounded-lg shadow-sm border hover:opacity-80 ${
            hasFeedbackMessage ? 'bg-black border-black' : 'bg-white border-gray-200 hover:bg-gray-100'
          }`}
          title="Add feedback message"
        >
          <MessageCircle className={`w-4 h-4 ${hasFeedbackMessage ? 'text-white fill-current' : 'text-gray-500'}`} />
        </button>
      </PopoverTrigger>
      <PopoverContent className="w-80 z-50" side="bottom" align="start">
        <div className="grid gap-4">
          <div className="space-y-2">
            <h4 className="font-medium leading-none">Enter the feedback for this message</h4>
          </div>
