...
export function transformTranscript(backendData: BackendTranscript): Transcript {
  try {
    ...
    const durationInSeconds =
      ...
      (lastEntry && transcriptArray[0] ? lastEntry.start_time - transcriptArray[0].start_time : 0);
    ...
    return {
      ...
      metrics: {
        ...
        tone: toneArray,
        wordCount:
          backendData.word_count ||
          transcriptArray.reduce((count, item) => count + (item.text?.split(/\s+/).length || 0), 0),
        in_progress_hostility_score: backendData.in_progress_hostility_score,
      },
